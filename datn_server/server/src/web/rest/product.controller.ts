...
export class ProductController {
  ...
  async post(@Req() req: Request, @Body() productDTO: ProductDTO): Promise<ProductDTO> {
    const created = await this.productService.save(productDTO, req.user?.email);
  ...
  async put(@Req() req: Request, @Body() productDTO: ProductDTO): Promise<ProductDTO> {
    ...
    return await this.productService.update(productDTO, req.user?.email);
  }

  @Put('/:id')
  @Roles(RoleType.ADMIN)
  @ApiOperation({ title: 'Update product with id' })
  @ApiResponse({
    status: 200,
    description: 'The record has been successfully updated.',
    type: ProductDTO,
