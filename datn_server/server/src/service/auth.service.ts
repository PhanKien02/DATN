...
export class AuthService {
  ...
  async login(userLogin: UserLoginDTO): Promise<any> {
    ...
    const userFind = await this.userService.findByFields({ where: { email: email } });
    ...
    const payload: Payload = { id: user.id, email: user.email, authorities: user.authorities };
  ...
  async registerNewUser(newUser: UserDTO): Promise<UserDTO> {
    let userFind: UserDTO = await this.userService.findByFields({ where: { email: newUser.email } });
    if (userFind) {
    ...
    const user: UserDTO = await this.userService.save(newUser, newUser.email, true);
    return user;
  }

  async updateUserSettings(userLogin: string, newUserInfo: UserDTO): Promise<UserDTO> {
    const userFind: UserDTO = await this.userService.findByFields({ where: { email: userLogin } });
    if (!userFind) {
      throw new HttpException('Invalid login name!', HttpStatus.BAD_REQUEST);
    }
